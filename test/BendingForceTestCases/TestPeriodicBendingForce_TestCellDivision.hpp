...
public:
    ...
    void TestPeriodicCubeWithGhosts() throw (Exception)
    {
        ...
        std::string output_directory = "PeriodicBend_TestCellDivision";
        ...
        double width_space = 0.9;
        double height_space = 0.9*sqrt(0.75);
        ...
        double beta_parameter = 0.50*spring_strength;
            ...
            {    
                for (unsigned i=0; i<width; i++)
                {
                    ...
                    if(cell_iter == 63 + 36)
                    {
                        ...
                        y_coordinate_test = y_coordinate - 0.1;
        ...
        // Add any extra nodes we may want.
        if(true)
        {
        x_coordinate = x_coordinate_test;
        y_coordinate = y_coordinate_test;
        z_coordinate = 5.738431453704834; 
        nodes.push_back(new Node<3>(cell_iter,  false,  x_coordinate, y_coordinate, z_coordinate));
        real_node_indices.push_back(cell_iter);
        // cell_iter++;
        }
        ...
        unsigned num_epithelial_cells = (width)*(height)+1;
        ...
        for (unsigned i=real_node_indices.size()-num_epithelial_cells; i<real_node_indices.size(); i++)
        {
            ...
            if( i==(63+36) || i==(real_node_indices.size()-1) )
            {
                PRINT_VARIABLE(i);
                birth_time = -0.1;
            }
...
//        cell_population.InitialiseCells();
         ...
         ++cell_iter)
        {
            ...
            cell_iter->GetCellData()->SetItem("cell_area", sqrt(3.0)/4.0);
        }
        TRACE("Done");
